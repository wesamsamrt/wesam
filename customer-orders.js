...
async function loadCustomerPreviousOrders() {
    ...
    const { data: orders, error } = await supabaseClient.from("orders").select("*").eq("user_id", user.id).neq("status", "جديد").order("id", { ascending: false });
    ...
    for (const order of orders) {
        ...
        card.innerHTML = `<div class="order-head"><div><h2>الطلب #${order.id}</h2><p class="date">${date}</p><p class="meta">🚚 ${escapeOrdersHtml(order.driver_name || "مندوب غير محدد")}</p></div><span class="status">${escapeOrdersHtml(order.status || "غير محدد")}</span></div><div class="items">${renderedItems}</div><div class="total"><span>إجمالي الطلب</span><strong>${Number(order.total || 0).toFixed(2)} ر.س</strong></div>`;
