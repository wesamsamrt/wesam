...
async function renderFinanceSystem(moduleKey = "overview") {
    ...
    financeSystemContent.innerHTML = '<div class="message">جاري تحميل البيانات المالية...</div>';
    const financeError = error => `<div class="message error">تعذر تحميل البيانات المالية: ${transferText(error.message)}<br><small>تأكد من تشغيل ملف add-finance-system.sql في Supabase.</small></div>`;
    const today = new Date().toISOString().slice(0, 10);
    try {
    if (moduleKey === "overview") {
        const { data, error } = await supabaseClient.rpc("list_finance_dashboard", { p_warehouse: selectedWarehouse });
        if (error) throw error;
        const stats = data || {};
        financeSystemContent.innerHTML = `<header class="finance-system-head"><div><h3>الرئيسية المالية</h3><p>نظام مستقل للحسابات، ولا يغيّر أي وظيفة في الطلبات أو المخزون.</p></div><span class="finance-system-badge">مخزن ${transferText(selectedWarehouse)}</span></header><div class="finance-kpis"><article><span>رصيد الحسابات</span><strong>${formatAdminCurrency(stats.accounts_balance || 0)}</strong></article><article><span>مصروفات هذا الشهر</span><strong>${formatAdminCurrency(stats.month_expenses || 0)}</strong></article><article><span>مستحقات الموردين</span><strong>${formatAdminCurrency(stats.suppliers_due || 0)}</strong></article><article><span>الحسابات النشطة</span><strong>${Number(stats.accounts_count || 0)}</strong></article></div><div class="finance-overview-grid">${Object.entries(financeModules).map(([key, module]) => `<button type="button" class="finance-overview-card" data-open-finance-module="${key}"><span>${module.icon}</span><h4>${module.title}</h4><p>${module.description}</p></button>`).join("")}</div>`;
    ...
    if (moduleKey === "cash") {
        const { data: accounts, error } = await supabaseClient.rpc("list_finance_accounts", { p_warehouse: selectedWarehouse });
        if (error) throw error;
        financeSystemContent.innerHTML = `<header class="finance-system-head"><div><h3>💵 الحسابات والصندوق</h3><p>أضف صندوقك النقدي أو حسابك البنكي، ثم تظهر أرصدته هنا.</p></div><span class="finance-system-badge">حسابات فعلية</span></header><form class="finance-form" id="financeAccountForm"><input name="name" required placeholder="اسم الحساب، مثال: صندوق الفرع"><select name="type"><option value="cash">صندوق نقدي</option><option value="bank">حساب بنكي</option></select><input name="opening" type="number" step="0.01" value="0" placeholder="الرصيد الافتتاحي"><input name="notes" placeholder="ملاحظات اختيارية"><button>+ إضافة حساب</button></form><div class="finance-records">${(accounts || []).length ? accounts.map(account => `<article><div><strong>${transferText(account.name)}</strong><small>${account.account_type === "bank" ? "حساب بنكي" : "صندوق نقدي"}</small></div><b>${formatAdminCurrency(account.current_balance || 0)}</b></article>`).join("") : '<div class="message">أضف أول صندوق أو حساب بنكي.</div>'}</div>`;
        financeSystemContent.querySelector("#financeAccountForm")?.addEventListener("submit", async event => { event.preventDefault(); const form = new FormData(event.currentTarget); const { error: saveError } = await supabaseClient.rpc("save_finance_account", { p_warehouse: selectedWarehouse, p_name: form.get("name"), p_type: form.get("type"), p_opening: Number(form.get("opening") || 0), p_notes: form.get("notes") || null }); if (saveError) return alert(saveError.message); renderFinanceSystem("cash"); });
        return;
    }
    if (moduleKey === "expenses") {
        const [{ data: accounts, error: accountsError }, { data: categories, error: categoriesError }, { data: expenses, error: expensesError }] = await Promise.all([supabaseClient.rpc("list_finance_accounts", { p_warehouse: selectedWarehouse }), supabaseClient.rpc("list_finance_expense_categories", { p_warehouse: selectedWarehouse }), supabaseClient.rpc("list_finance_expenses", { p_warehouse: selectedWarehouse })]);
        if (accountsError || categoriesError || expensesError) throw accountsError || categoriesError || expensesError;
        financeSystemContent.innerHTML = `<header class="finance-system-head"><div><h3>🧾 المصروفات</h3><p>سجل كل مصروف ويخصم تلقائيًا من الحساب الذي اخترته.</p></div><span class="finance-system-badge">سجل المصروفات</span></header><form class="finance-form" id="financeCategoryForm"><input name="name" required placeholder="إضافة تصنيف، مثال: شحن"><button>+ تصنيف</button></form><form class="finance-form" id="financeExpenseForm"><select name="account" required><option value="">اختر الحساب</option>${(accounts || []).map(a => `<option value="${a.id}">${transferText(a.name)}</option>`).join("")}</select><select name="category"><option value="">غير مصنف</option>${(categories || []).map(c => `<option value="${c.id}">${transferText(c.name)}</option>`).join("")}</select><input name="amount" required min="0.01" step="0.01" type="number" placeholder="المبلغ"><input name="date" type="date" value="${today}"><input name="payee" placeholder="الجهة المستلمة"><button>+ تسجيل مصروف</button></form><div class="finance-records">${(expenses || []).map(item => `<article><div><strong>${transferText(item.category)}</strong><small>${transferText(item.account)} · ${transferText(item.expense_date)}</small></div><b class="negative">-${formatAdminCurrency(item.amount)}</b></article>`).join("") || '<div class="message">لا توجد مصروفات مسجلة.</div>'}</div>`;
        financeSystemContent.querySelector("#financeCategoryForm")?.addEventListener("submit", async e => { e.preventDefault(); const name = new FormData(e.currentTarget).get("name"); const { error: saveError } = await supabaseClient.rpc("save_finance_expense_category", { p_warehouse: selectedWarehouse, p_name: name }); if (saveError) return alert(saveError.message); renderFinanceSystem("expenses"); });
        financeSystemContent.querySelector("#financeExpenseForm")?.addEventListener("submit", async e => { e.preventDefault(); const f = new FormData(e.currentTarget); const { error: saveError } = await supabaseClient.rpc("save_finance_expense", { p_warehouse: selectedWarehouse, p_account_id: Number(f.get("account")), p_category_id: f.get("category") ? Number(f.get("category")) : null, p_amount: Number(f.get("amount")), p_date: f.get("date"), p_payee: f.get("payee") || null, p_notes: null }); if (saveError) return alert(saveError.message); renderFinanceSystem("expenses"); });
        return;
    }
    if (moduleKey === "suppliers") {
        const { data: suppliers, error } = await supabaseClient.rpc("list_finance_suppliers", { p_warehouse: selectedWarehouse });
        if (error) throw error;
        financeSystemContent.innerHTML = `<header class="finance-system-head"><div><h3>🚚 الموردون والمشتريات</h3><p>سجل الموردين وارصدتهم المستحقة في النظام المالي.</p></div><span class="finance-system-badge">سجل الموردين</span></header><form class="finance-form" id="financeSupplierForm"><input name="name" required placeholder="اسم المورد"><input name="phone" placeholder="رقم الجوال"><input name="email" type="email" placeholder="البريد الإلكتروني"><input name="address" placeholder="العنوان"><button>+ إضافة مورد</button></form><div class="finance-records">${(suppliers || []).map(item => `<article><div><strong>${transferText(item.name)}</strong><small>${transferText(item.phone || "بدون رقم")}</small></div><b>${formatAdminCurrency(item.due_balance || 0)}</b></article>`).join("") || '<div class="message">لا يوجد موردون مسجلون.</div>'}</div>`;
        financeSystemContent.querySelector("#financeSupplierForm")?.addEventListener("submit", async e => { e.preventDefault(); const f = new FormData(e.currentTarget); const { error: saveError } = await supabaseClient.rpc("save_finance_supplier", { p_warehouse: selectedWarehouse, p_name: f.get("name"), p_phone: f.get("phone") || null, p_email: f.get("email") || null, p_address: f.get("address") || null, p_notes: null }); if (saveError) return alert(saveError.message); renderFinanceSystem("suppliers"); });
        return;
    }
    if (moduleKey === "dues") {
        const [{ data: dues, error: duesError }, { data: accounts, error: accountsError }] = await Promise.all([supabaseClient.rpc("list_finance_customer_dues", { p_warehouse: selectedWarehouse }), supabaseClient.rpc("list_finance_accounts", { p_warehouse: selectedWarehouse })]);
        if (duesError || accountsError) throw duesError || accountsError;
        financeSystemContent.innerHTML = `<header class="finance-system-head"><div><h3>📒 الذمم</h3><p>مستحقات العملاء من التحضيرات، وسجل التحصيل المالي.</p></div><span class="finance-system-badge">ذمم العملاء</span></header><form class="finance-form" id="financePaymentForm"><input name="customer" required placeholder="اسم العميل"><select name="account" required><option value="">حساب التحصيل</option>${(accounts || []).map(a => `<option value="${a.id}">${transferText(a.name)}</option>`).join("")}</select><input name="amount" required min=".01" step=".01" type="number" placeholder="المبلغ"><input name="date" type="date" value="${today}"><button>تسجيل تحصيل</button></form><div class="finance-records">${(dues || []).map(d => `<article><div><strong>${transferText(d.customer_name)}</strong><small>الإجمالي ${formatAdminCurrency(d.order_total)} · المسدد ${formatAdminCurrency(d.paid_amount)}</small></div><b class="negative">${formatAdminCurrency(d.due_amount)}</b></article>`).join("") || '<div class="message">لا توجد ذمم عملاء.</div>'}</div>`;
        financeSystemContent.querySelector("#financePaymentForm")?.addEventListener("submit", async e => { e.preventDefault(); const f = new FormData(e.currentTarget); const { error: saveError } = await supabaseClient.rpc("save_finance_customer_payment", { p_warehouse: selectedWarehouse, p_customer_name: f.get("customer"), p_account_id: Number(f.get("account")), p_amount: Number(f.get("amount")), p_date: f.get("date"), p_notes: null }); if (saveError) return alert(saveError.message); renderFinanceSystem("dues"); });
        return;
    }
    if (moduleKey === "pricing") {
        const { data: lists, error } = await supabaseClient.rpc("list_finance_price_lists", { p_warehouse: selectedWarehouse });
        if (error) throw error;
        financeSystemContent.innerHTML = `<header class="finance-system-head"><div><h3>🏷️ الأسعار والخصومات</h3><p>قوائم سعر مستقلة للتجزئة والجملة وVIP.</p></div><span class="finance-system-badge">قوائم الأسعار</span></header><form class="finance-form" id="financePriceListForm"><input name="name" required placeholder="اسم القائمة: جملة أو VIP"><select name="mode"><option value="fixed">مبلغ ثابت</option><option value="percentage">نسبة مئوية</option></select><input name="adjustment" type="number" step=".01" value="0" placeholder="الزيادة أو الخصم"><button>+ حفظ القائمة</button></form><div class="finance-records">${(lists || []).map(l => `<article><div><strong>${transferText(l.name)}</strong><small>${l.price_mode === "percentage" ? "نسبة مئوية" : "مبلغ ثابت"}</small></div><b>${Number(l.adjustment || 0)}</b></article>`).join("") || '<div class="message">لا توجد قوائم سعر.</div>'}</div>`;
        financeSystemContent.querySelector("#financePriceListForm")?.addEventListener("submit", async e => { e.preventDefault(); const f = new FormData(e.currentTarget); const { error: saveError } = await supabaseClient.rpc("save_finance_price_list", { p_warehouse: selectedWarehouse, p_name: f.get("name"), p_mode: f.get("mode"), p_adjustment: Number(f.get("adjustment") || 0) }); if (saveError) return alert(saveError.message); renderFinanceSystem("pricing"); });
        return;
    }
    if (moduleKey === "reports") {
        const { data, error } = await supabaseClient.rpc("list_finance_dashboard", { p_warehouse: selectedWarehouse }); if (error) throw error;
        financeSystemContent.innerHTML = `<header class="finance-system-head"><div><h3>📊 المحاسبة والتقارير</h3><p>ملخص مباشر لأرقام النظام المالي.</p></div><span class="finance-system-badge">تقارير مالية</span></header><div class="finance-kpis"><article><span>رصيد الحسابات</span><strong>${formatAdminCurrency(data?.accounts_balance || 0)}</strong></article><article><span>مصروفات الشهر</span><strong>${formatAdminCurrency(data?.month_expenses || 0)}</strong></article><article><span>ذمم الموردين</span><strong>${formatAdminCurrency(data?.suppliers_due || 0)}</strong></article></div>`; return;
    }
    if (moduleKey === "staff") {
        financeSystemContent.innerHTML = `<header class="finance-system-head"><div><h3>🔐 صلاحيات الموظفين</h3><p>إدارة الموظفين موجودة في صفحة الحسابات والصلاحيات الحالية، ويمكن منحهم إذن «الإدارة المالية» منها.</p></div><span class="finance-system-badge">صلاحيات مستقلة</span></header><button type="button" class="open-invoice-button" id="openFinancialPermissions">فتح الحسابات والصلاحيات</button>`;
    ...
    if (moduleKey === "pos") {
        ...
        financeSystemContent.innerHTML = `<header class="finance-system-head"><div><h3>🛒 نقطة البيع</h3><p>فاتورة بيع مباشرة مستقلة، تخصم من المخزون وتضيف المبلغ إلى الحساب المختار.</p></div><span class="finance-system-badge">كاشير</span></header><div class="finance-form"><input id="financePosSearch" placeholder="ابحث بالكود أو اسم المنتج"><select id="financePosProduct"><option value="">اختر المنتج</option>${list.map(p=>`<option value="${p.id}">${transferText(p.product_code||'—')} · ${transferText([p.company,p.type,p.model,p.color].filter(Boolean).join(' '))} (المتاح ${p.quantity})</option>`).join('')}</select><input id="financePosQty" type="number" min="1" value="1"><input id="financePosPrice" type="number" min="0" step=".01" placeholder="السعر"><button type="button" id="financePosAdd">إضافة للسلة</button></div><form id="financePosForm"><div class="finance-form"><input name="customer" placeholder="اسم العميل اختياري"><select name="account" required><option value="">حساب الاستلام</option>${(accounts||[]).map(a=>`<option value="${a.id}">${transferText(a.name)}</option>`).join('')}</select><select name="method"><option>نقدي</option><option>شبكة</option><option>تحويل</option><option>آجل</option></select><button>إصدار فاتورة البيع</button></div><div class="finance-records" id="financePosCart"></div></form>`;
        const productSelect=financeSystemContent.querySelector('#financePosProduct'), search=financeSystemContent.querySelector('#financePosSearch'); search.addEventListener('input',()=>{const q=search.value.toLowerCase(); [...productSelect.options].forEach(o=>{if(!o.value)return;o.hidden=!o.text.toLowerCase().includes(q);});}); financeSystemContent.querySelector('#financePosAdd').addEventListener('click',()=>{const p=list.find(x=>String(x.id)===productSelect.value), qty=Number(financeSystemContent.querySelector('#financePosQty').value||0), price=Number(financeSystemContent.querySelector('#financePosPrice').value||p?.price||0); if(!p||qty<1||qty>Number(p.quantity))return alert('اختر صنفًا وكمية متاحة.'); const existing=cart.find(i=>i.product_id===p.id); if(existing)existing.quantity+=qty;else cart.push({product_id:p.id,name:[p.company,p.type,p.model,p.color].filter(Boolean).join(' · '),quantity:qty,price});renderCart();}); financeSystemContent.querySelector('#financePosForm').addEventListener('submit',async e=>{e.preventDefault();if(!cart.length)return alert('السلة فارغة.');const f=new FormData(e.currentTarget);const {data,error:saveError}=await supabaseClient.rpc('create_finance_pos_sale',{p_sale:{warehouse:selectedWarehouse,customer_name:f.get('customer'),account_id:Number(f.get('account')),payment_method:f.get('method'),items:cart}});if(saveError)return alert(saveError.message);alert(`تم إصدار فاتورة البيع #${data}`);renderFinanceSystem('pos');}); renderCart(); return;
    }
    ...
    } catch (error) { console.error("Finance module error:", error); financeSystemContent.innerHTML = financeError(error); }
}
...
let manualReturnCatalogProducts = [];
...
function newManualReturnItem(isExternal = false) {
    return { product_code: "", category: "", product_type: "", type: "", company: "", model: "", color: "", storage_location: "", quantity: 1, price: 0, prepared_quantity: 0, max_return_quantity: isExternal ? null : 0, is_external: isExternal };
}
...
function getManualReturnSourceItems(item = null) {
    return item?.is_external ? manualReturnCatalogProducts : (getManualReturnCustomer()?.items || []);
}
...
function getManualReturnCandidates(item) {
    let candidates = getManualReturnSourceItems(item);
...
function copyManualReturnProduct(index, product) {
    ...
    const isExternal = Boolean(current.is_external);
    const quantities = isExternal ? { prepared: 0, available: null } : getManualReturnQuantities(product);
    manualReturnItemsData[index] = {
        ...
        quantity: isExternal ? Math.max(1, Number(current.quantity || 1)) : quantities.available
    };
}
...
function renderManualReturnItems() {
    ...
    manualReturnItems.innerHTML = manualReturnItemsData.map((item, index) => {
        ...
        const sourceItems = getManualReturnSourceItems(item);
    ...
    manualReturnItems.querySelectorAll("[data-manual-return-field]").forEach(input => input.addEventListener("input", event => {
        ...
        const isExternal = Boolean(manualReturnItemsData[index].is_external);
        const allowed = field === "quantity" && !isExternal ? Math.max(0, Number(manualReturnItemsData[index].max_return_quantity || 0)) : numericValue;
        manualReturnItemsData[index][field] = ["quantity", "price"].includes(field) ? Math.min(numericValue, allowed) : event.currentTarget.value;
        if (field === "quantity" && !isExternal && numericValue > allowed) {
...
async function loadManualReturnCatalog() {
    const products = [];
    const pageSize = 1000;
    for (let from = 0; ; from += pageSize) {
        const { data, error } = await supabaseClient
            .from("products")
            .select("id, product_code, category, product_type, type, company, model, color, storage_location, price")
            .eq("warehouse", selectedWarehouse)
            .order("id", { ascending: true })
            .range(from, from + pageSize - 1);
        if (error) { setManualReturnMessage(`تعذر تحميل أصناف المخزن: ${error.message}`, true); return; }
        products.push(...(data || []));
        if (!data || data.length < pageSize) break;
    }
    manualReturnCatalogProducts = products;
}
...
function openManualReturnEditor() {
    ...
    manualReturnCatalogProducts = [];
    setManualReturnMessage(""); renderManualReturnItems();
    Promise.all([loadManualReturnCustomers(), loadManualReturnCatalog()]).then(() => renderManualReturnItems());
}
...
async function saveManualReturn() {
    ...
    const items = manualReturnItemsData.map(item => ({ ...item, product_code: String(item.product_code || "").trim(), quantity: Number(item.quantity || 0), price: Number(item.price || 0), outside_preparation: Boolean(item.is_external) })).filter(item => item.product_code && item.quantity > 0);
    if (!items.length) { setManualReturnMessage("أضف منتجًا واحدًا صحيحًا على الأقل مع الكمية.", true); return; }
    const requestedByProduct = new Map();
    items.filter(item => !item.is_external).forEach(item => {
...
document.getElementById("addExternalManualReturnItem")?.addEventListener("click", async () => {
    if (!manualReturnCatalogProducts.length) await loadManualReturnCatalog();
    manualReturnItemsData.push(newManualReturnItem(true));
    renderManualReturnItems();
});
...
async function openShortageProductStats(selectedProduct = null) {
    ...
    try {
        ...
        const monthDailyRate = monthSales / 30;
